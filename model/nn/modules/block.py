...
__all__ = (
    ...
    "SCDown",
    "TorchVision",
    "MANet","ESIM","PSConv","SNI","GSConvE","F_Add","C2PSA_EPGO","SRFD","AFGCAttention","MLCA","C2PSA_MALA","C2TSSA","EIEStem",
    "FeaturePyramidSharedConv","GCConv","SPPF_ScaleAware","DAC","C3k2_MambaOut","DilatedConv","CDC","SPDConv","ODConv","WTConv","SCConv",
    "YOLO_DSConv"
)
...
# class DAC(nn.Module):
#     """
#     DAC_OptPlus: Structure-Aware Anisotropic Convolution (Final Version)
#
#     Key features:
...
class DAC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        ...
        # ------------------------------------------------------
        # Strategy: Bottleneck for channel mixing (DAC1 idea) + Residual for flow (DAC idea)

        # Calculate reduction ratio (e.g., squeeze factor 4)
        mid_channels = max(8, in_channels // 4)

        # Main nonlinear path (Bottleneck style from DAC1 for better channel mixing)
        self.gain_map_generator = nn.Sequential(
        ...
        # Residual linear shortcut (from DAC for gradient stability)
    ...
    def get_local_mad(x, k=3, stride=1):
        """
        Compute MAD with 'High-Res Diff' strategy from DAC.
...
class GatedCNNBlock_BCHW(nn.Module):
    ...
    def __init__(self, dim, expansion_ratio=8/3, kernel_size=7, conv_ratio=1.0,
        ...
        self.conv = DAC(conv_channels, conv_channels, kernel_size=kernel_size, padding=kernel_size//2)
